...
QString UrlFactory::MakeImageUrl(const MapType::Types &type, const Point &pos, const int &zoom, const QString &language) {
    ...
    case MapType::PergoTurkeyMap: {
        ...
        return QString("http://map%1.pergo.com.tr/tile/%2/%3/%4.png")
               .arg(GetServerNum(pos, 4)).arg(zoom, 2, 10, (QChar)'0').arg(x).arg(y);
    }
    break;
    case MapType::SigPacSpainMap: {
        return QString("http://sigpac.mapa.es/kmlserver/raster/%1@3785/%2.%3.%4.img")
               .arg(levelsForSigPacSpainMap[zoom]).arg(zoom).arg(pos.X()).arg((2 << (zoom - 1)) - pos.Y() - 1);
    }
    break;
    ...
    case MapType::YandexMapRu: {
        ...
        return QString("http://%1").arg(server) + QString("0%2.maps.yandex.ru/tiles?l=map&v=%3&x=%4&y=%5&z=%6")
               .arg(GetServerNum(pos, 4) + 1).arg(VersionYandexMap).arg(pos.X()).arg(pos.Y()).arg(zoom);
    }
    break;
    case MapType::Statkart_Topo2: {

        return QString("http://opencache.statkart.no/gatekeeper/gk/gk.open_gmaps?layers=topo2&zoom=%1&x=%2&y=%3")
               .arg(zoom).arg(pos.X()).arg(pos.Y());
    }
    break;

    case MapType::AMap_Normal: {
        // 矢量地图（最新可用）
        return QString("https://wprd0%1.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scl=1&style=7&x=%2&y=%3&z=%4")
               .arg(GetServerNum(pos, 4) + 1).arg(pos.X()).arg(pos.Y()).arg(zoom);
    }
    break;
    case MapType::AMap_Satellite: {
        return QString("https://webst0%1.is.autonavi.com/appmaptile?style=6&x=%2&y=%3&z=%4")
               .arg(GetServerNum(pos, 4) + 1).arg(pos.X()).arg(pos.Y()).arg(zoom);
    }
    break;
    case MapType::AMap_Hybrid: {
        return QString("https://webst0%1.is.autonavi.com/appmaptile?style=8&x=%2&y=%3&z=%4")
               .arg(GetServerNum(pos, 4) + 1).arg(pos.X()).arg(pos.Y()).arg(zoom);
    }
    break;
    default:
